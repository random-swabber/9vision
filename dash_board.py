...
with col2:
    st.markdown('<h3>Upvotes '+arrow_up+'</h3>', unsafe_allow_html=True)
    st.markdown('<br>'.join(df_reactions.iloc[0].astype('int64').astype('str').to_list()), unsafe_allow_html=True)

with col3:
    st.markdown('<h3>Downvotes '+arrow_down+'</h3>', unsafe_allow_html=True)
